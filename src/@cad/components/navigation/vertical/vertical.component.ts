...
import { CadNavigationItem, CadVerticalNavigationAppearance, CadVerticalNavigationMode, CadVerticalNavigationPosition } from '@cad/components/navigation/navigation.types';
...
@Component({
    selector       : 'cad-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : cadAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'cadVerticalNavigation'
})
export class CadVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: CadVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: CadVerticalNavigationMode = 'side';
    @Input() name: string = this._cadUtilsService.randomId();
    @Input() navigation: CadNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: CadVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<CadVerticalNavigationAppearance> = new EventEmitter<CadVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<CadVerticalNavigationMode> = new EventEmitter<CadVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<CadVerticalNavigationPosition> = new EventEmitter<CadVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<CadNavigationItem> = new ReplaySubject<CadNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<CadNavigationItem> = new ReplaySubject<CadNavigationItem>(1);
    ...
    private _cadScrollbarDirectives!: QueryList<CadScrollbarDirective>;
    private _cadScrollbarDirectivesSubscription: Subscription;
    ...
    {
        return {
            'cad-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`cad-vertical-navigation-appearance-${this.appearance}`]: true,
            'cad-vertical-navigation-hover'                          : this._hovered,
            'cad-vertical-navigation-inner'                          : this.inner,
            'cad-vertical-navigation-mode-over'                      : this.mode === 'over',
            'cad-vertical-navigation-mode-side'                      : this.mode === 'side',
            'cad-vertical-navigation-opened'                         : this.opened,
            'cad-vertical-navigation-position-left'                  : this.position === 'left',
            'cad-vertical-navigation-position-right'                 : this.position === 'right'
        };
    }
    ...
    /**
     * Setter for cadScrollbarDirectives
     */
    @ViewChildren(CadScrollbarDirective)
    set cadScrollbarDirectives(cadScrollbarDirectives: QueryList<CadScrollbarDirective>)
    {
        // Store the directives
        this._cadScrollbarDirectives = cadScrollbarDirectives;

        // Return if there are no directives
        if ( cadScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._cadScrollbarDirectivesSubscription )
        {
            this._cadScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._cadScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    cadScrollbarDirectives.forEach((cadScrollbarDirective) => {
                        cadScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        setTimeout(() => {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.cad-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._cadScrollbarDirectives.forEach((cadScrollbarDirective) => {

                    // Skip if not enabled
                    if ( !cadScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    cadScrollbarDirective.scrollToElement('.cad-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: CadNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: CadNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('cad-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('cad-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('cad-vertical-navigation-aside-overlay');
