...
import { ScrollbarGeometry, ScrollbarPosition } from '@cad/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[cadScrollbar]',
    exportAs: 'cadScrollbar'
})
export class CadScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_cadScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() cadScrollbar: boolean = true;
    @Input() cadScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'cadScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.cadScrollbar = coerceBooleanProperty(changes.cadScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.cadScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'cadScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.cadScrollbarOptions.currentValue);
